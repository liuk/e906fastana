...
int main(int argc, char* argv[])
{
	...
	//manual bining
	const double x1min = 0.3;
	const double x1max = 1.0;
	const double x1bin[] = {x1min, 0.5, 0.6, 0.7, 0.8, 0.9, x1max};
	const int nx1bins = 6;
	...
	for(int i = 0; i < dataTree->GetEntries(); ++i)
	{
		...
		//if(event.runID > 10000) continue;
		if(dimuon.mass < 4.2) continue;
		if(dimuon.x1 < x1min || dimuon.x1 > x1max) continue;

		fout << event.runID << " " << event.spillID << " " << event.eventID << endl;
	...
	for(int i = 0; i < nx1bins; ++i)
	{
		R_np[i] = A[3]*xsec[3][i]/xsec[1][i] - 1.;
	...
	for(int i = 1; i <= nTargets; ++i)
	{
		cout << " ============= On Target " << targetName[i] << " with " << NProtons[i] << " live protons =============" << endl;
		for(int j = 0; j < nx1bins; ++j)
		{
			cout << " --- x1 bin " << j << " : " << x1bin[j] << " --- " << x1bin[j+1] << " --- " << endl;
			
			cout << nevt[i][j] << " events. x1_avg = " << x1_avg[i][j] << ", ";
	...
	double dyl[nTargets+1][nx1bins] = {0};
	double dyr[nTargets+1][nx1bins] = {0};
	...
	for(int i = 1; i <= nTargets; ++i)
	{
		for(int j = 0; j < nx1bins; ++j)
		{
			...
			dyl[i][j] = err_xsec_ratio_corr[i][j];
			dyr[i][j] = err_xsec_ratio_corr[i][j];
		}

		gr[i] = new TGraphAsymmErrors(nx1bins, x[i], y[i], dxl[i], dxr[i], dyl[i], dyr[i]);
	...
	c1->cd(1); gr[6]->Draw("AP"); f.Draw("same");
	c1->cd(2); gr[5]->Draw("AP"); f.Draw("same");
	c1->cd(3); gr[7]->Draw("AP"); f.Draw("same");
