...
int main(int argc, char* argv[])
{
    ...
    spill.mcflag = false;
    if(server->HasTable("mDimuon")) spill.mcflag = true;
    ...
    for(int i = 0; i < nDimuonsRaw; ++i)
    {
        ...
        //spill info and cuts
        if(!spill.mcflag && event.spillID > spill.spillID) //we have a new spill here
        {
    ...
    if(!spill.mcflag) cout << nGoodSpill << " good spills, " << nBadSpill_record << " spills have insufficient info in database, " << nBadSpill_quality << " rejected because of bad quality." << endl;
